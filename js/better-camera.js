...
XML3D.util.ActiveViewObserver = function(target, callback) {
...
XML3D.util.ActiveViewObserver.prototype.dispatch = function(mutations) {
	mutations.forEach(function(mutation) {
		console.log(mutation);
		if (mutation.type != 'attributes') return;
		if (mutation.attributeName != 'activeview') return;
		this.cb.call(this);
	}); 
}
...
XML3D.camera.EventDispatcher = function(xml3d) {
	this.activeView = null;
	this.xml3d = xml3d || null;
	
	this.attach(this.xml3d);
}


XML3D.camera.EventDispatcher.prototype.stopEvent = function(ev) {
...
XML3D.camera.Interaction = {
	screen_start: 'interaction_screen_start',
	screen_move: 'interaction_screen_move',
	screen_end: 'interaction_screen_end',
	
	button_down: 'interaction_button_down',
	button_up: 'interaction_button'
};


var generateEventHandler = function(eventType, eventData) {
	return function(event) {
		if (this.activeView === null) return;
		var ev = event || window.event;
		var interactionEvent = new CustomEvent(eventType, {'detail': eventData(ev) });
		return this.activeView.dispatchEvent(interactionEvent);
	};
};

var mouseEventData = function(ev) {
	return {
		x: ev.pageX,
		y: ev.pageY,
		originEvent: ev
...
XML3D.camera.EventDispatcher.prototype.mousePressEvent = generateEventHandler(XML3D.camera.Interaction.screen_start, mouseEventData);
XML3D.camera.EventDispatcher.prototype.mouseReleaseEvent = generateEventHandler(XML3D.camera.Interaction.screen_end, mouseEventData);
XML3D.camera.EventDispatcher.prototype.mouseMoveEvent = generateEventHandler(XML3D.camera.Interaction.screen_move, mouseEventData);

var touchEventData = function(ev) {
	return {
		x: ev.pageX,
		y: ev.pageY,
		originEvent: ev
...
XML3D.camera.EventDispatcher.prototype.touchStartEvent = generateEventHandler(XML3D.camera.Interaction.screen_start, touchEventData);
XML3D.camera.EventDispatcher.prototype.touchEndEvent = generateEventHandler(XML3D.camera.Interaction.screen_end, touchEventData);
XML3D.camera.EventDispatcher.prototype.touchMoveEvent = generateEventHandler(XML3D.camera.Interaction.screen_move, touchEventData);

XML3D.camera.EventDispatcher.prototype.getView = function() {
	var activeViewId = this.xml3d.activeView;
	XML3D.debug.logInfo("Active View: " + activeViewId);
	var activeView = null;

	if (activeViewId)
	{
		if (activeViewId.substring(0, 1) == '#') {
			activeViewId = activeViewId.substring(1);
		}
		XML3D.debug.logInfo("Trying to resolve view '" + activeViewId +"'");
		activeView = document.getElementById(activeViewId);
	}
...
XML3D.camera.EventDispatcher.prototype.viewChanged = function() {
	this.activeView = this.getView();
	// console.log('View changed: ' + this.activeView.id);
	return false;
}

XML3D.camera.EventDispatcher.prototype.attach = function(xml3d) {
	if (!xml3d) return;
	
	// for (var prop in view.orientation)
	...
	xml3d.addEventListener("mousedown", this._evt_mousedown, false);
	document.addEventListener("mouseup", this._evt_mouseup, false);
	document.addEventListener("mousemove",this._evt_mousemove, false);

	xml3d.addEventListener("touchstart", this._evt_touchstart, false);
	...
	this._evt_viewchanged = function() { self.viewChanged(); };
	this.viewObserver = new XML3D.util.ActiveViewObserver(xml3d, this._evt_viewchanged);
	// trigger once on xml3d initialization
	self.viewChanged();
};
...
XML3D.camera.MouseButton = {
	Left: 1,
	Right: 2,
	Middle: 3
};

XML3D.camera.Navigation = function(view) {
	this.view = view || null;
	this.xml3d = null;
	
	this.upAxis = new window.XML3DVec3(0, 1, 0);
	
	this.from = this.view.position;
	this.focal = new window.XML3DVec3(0, 0, 0);
...
XML3D.camera.Navigation.prototype.stopEvent = function(ev) {
	if (ev.preventDefault)
		ev.preventDefault();
	if (ev.stopPropagation)
		ev.stopPropagation();
...
XML3D.camera.Navigation.prototype.update = function() {
	if (!this.needUpdate) return;
	this.needUpdate = false;

	var dir = this.focal.subtract(this.from);
	this.view.setDirection(dir);

	// TODO: view element need a setPosition()
	var str = this.from.x + " " + this.from.y + " " + this.from.z;
	this.view.setAttribute("position", str);

	if (!this.xml3d.update) return;
	this.xml3d.update();
};

XML3D.camera.Navigation.prototype.screenStart = function(ev) {
	console.log(ev);
	return false;
}

XML3D.camera.Navigation.prototype.screenMove = function(ev) {
	console.log(ev);
	return false;
}

XML3D.camera.Navigation.prototype.screenEnd = function(ev) {
	console.log(ev);
	return false;
}

XML3D.camera.Navigation.prototype.attach = function(view) {
	if (!view) return;
	...
	while (curNode.tagName != 'XML3D') {
		console.log(curNode);
		curNode = curNode.parentNode;
	}
		
	this.xml3d = curNode;
	console.log(this.xml3d);
	
	var self = this;
	this._evt_screenstart = function(e) { self.screenStart(e); };
	this._evt_screenend = function(e) { self.screenEnd(e); };
	this._evt_screenmove = function(e) { self.screenMove(e); };
	// this._evt_contextmenu = function(e) {self.stopEvent(e);};
	// this._evt_keydown = function(e) {self.keyHandling(e);};
	...
	view.addEventListener(XML3D.camera.Interaction.screen_start, this._evt_screenstart, false);
	view.addEventListener(XML3D.camera.Interaction.screen_end, this._evt_screenend, false);
	view.addEventListener(XML3D.camera.Interaction.screen_move, this._evt_screenmove, false);
...
XML3D.camera.Trackball = function(view) {
	XML3D.camera.Navigation.call(this, view);
...
XML3D.camera.Trackball.prototype = Object.create(XML3D.camera.Navigation.prototype);

XML3D.camera.Trackball.prototype.screenStart = function(ev) {
	var x = ev.detail.x;
	var y = ev.detail.y;
	
	console.log("Start trackball at " + x + ", " + y);
	
	console.log(this.from._data);
	console.log(this.focal._data);

	var button = (ev.detail.originEvent.which || ev.detail.originEvent.button);

	// check for left mouse button
	// TODO: refactor and generalize check
	this.active = (button == XML3D.camera.MouseButton.Left);

	this.prevPos.x = ev.detail.x;
	this.prevPos.y = ev.detail.y;

	// this.stopEvent(ev);
	return false;
}

XML3D.camera.Trackball.prototype.screenEnd = function(ev) {
	var x = ev.detail.x;
	var y = ev.detail.y;
	this.active = false;
	console.log("Finish trackball at " + x + ", " + y);
	return false;
}

XML3D.camera.Trackball.prototype.screenMove = function(ev) {
	if (!this.active)
		return;

	var dX = ev.detail.x - this.prevPos.x;
	var dY = ev.detail.y - this.prevPos.y;
	console.log("Motion: " + dX + ", " + dY);

	this.needUpdate = this.mapMotion(dX, dY);
	if (!this.needUpdate) return;
...
XML3D.camera.Trackball.prototype.mapMotion = function(dX, dY) {

	var dx = -this.rotateSpeed * dX * 2.0 * Math.PI / this.xml3d.width;
	var dy = -this.rotateSpeed * dY * 2.0 * Math.PI / this.xml3d.height;
	...
	var q0 = mx.multiply(my);
	var p0 = this.focal;
	
	var tmp = this.view.orientation.multiply(q0);
	tmp.normalize();
	// console.log(q0.axis._data);
	var invTf = tmp.rotateVec3(q0.axis);
	// console.log(invTf._data);
	var trans = new window.XML3DRotation(invTf, q0.angle).rotateVec3(this.from.subtract(p0));
	this.from = p0.add(trans);
...
var onload = function() {
	...
	// console.log(xml3dList);
	XML3D.camera.dispatcher = new Array();
	for (var i in xml3dList) {
		// XML3D.debug.logInfo("Creating EventDispatcher for xml3d element '" + i.getAttribute("id") + "'.");
		XML3D.camera.dispatcher[i] = new XML3D.camera.EventDispatcher(xml3dList[i]);
	};
	// console.log(XML3D.camera.dispatcher);
};

var onunload = function() {
	// for (var i in XML3D.camera.dispatcher)
	// {
		// XML3D.camera.controllers[i].detach();
	// }
};
