...
XML3D.Terrain.prototype.draw_tiles = function(tiles,layers){
	...
	for(key in tiles){
		...
		for (tile_key in needed_tiles){
			if(index<freetiles.length){
				...
				for(var j=0;j<layers.length;j++){
					...
					//reuse old tile
					group.children[freetiles[index]*layers.length+j].setAttribute("id", tile_key + layers[j]);
					group.children[freetiles[index]*layers.length+j].setAttribute("src", tile_key + "#" + layers[j]);
					group.children[freetiles[index]*layers.length+j].setAttribute("transform", tile_key + "#tf");
					this.updatedtiles++;
					this.reusedtiles++;
				}
				index++;
		...
		while(index<freetiles.length){
			...
			for(var i=0;i<layers.length;i++){
				var tile=group.children[(tile_index-deleted)*layers.length];
